...
export default function InterviewPage() {
  ...
  const submitAnswer = async () => {
    ...
    try {
      ...
      if (feedback.status === "completed") {
        ...
        setCurrentSession({
          ...currentSession,
          first_question: feedback.next_question,
          current_question_number: feedback.question_number
        })
        
        // Speak next question
        speakQuestion(feedback.next_question.question)
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    if (sessionStarted && !interviewComplete && recordingMode === "text") {
      ...
      if (currentSession?.first_question) {
        setTimeout(() => {
          speakQuestion(
            typeof currentSession.first_question === 'string'
              ? currentSession.first_question
              : currentSession.first_question.question
          )
        }, 1000)
      }
    }
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">
                  Question {currentSession?.current_question_number} of {currentSession?.question_count}
                </p>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-lg font-medium">
                        {currentSession.first_question?.question || currentSession.first_question}
                      </p>
                      {currentSession.first_question?.difficulty && (
                        <p className="text-sm text-muted-foreground mt-2">
                          Difficulty: <span className="font-medium capitalize">{currentSession.first_question.difficulty}</span>
                        </p>
                      )}
                    </div>
                  </div>
